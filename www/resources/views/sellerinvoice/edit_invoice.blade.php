...
@section('title') Edit Seller Invoice @endsection
@section('css')
...
@section('content')
    ...
    @component('common-components.breadcrumb',['li_1'=>['Dashboard'=>route('home'),'Seller Invoice List' =>route('sellerinvoice.index') ]])
        @slot('title') Edit Seller Invoice  @endslot
    @endcomponent
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <br/>
                    {!! Form::open(['url' => route('sellerinvoices.update',$invoice->id),'id'=>'edit-seller-invoice-form','method'=>'post']) !!}
                    @method('PATCH')
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            ...
                                            {!! Form::text('name',$invoice->name,['class' => 'form-control','placeholder'=>'Enter seller name','required'=>true]); !!}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                                        ...
                                                        @if($products_with_size_data)
                                                            @foreach($products_with_size_data as $row)
                                                                <option value="{{$row['product_name']}}"
                                                                        data-size="{{json_encode($row['size'])}}"
                                                                        data-id="{{$row['product_id']}}" {{$row['product_name'] == $item_row->name ?'selected':''}}>{{$row['product_name']}}</option>
                                                            @endforeach
                                                        @endif
                                                    </select>
                                                    <input type="hidden" name="item[{{$key}}][product_id]"
                                                           value="{{$item_row->product_id}}"
                                                           class="item-product-id form-control" required>
                                                </td>
                                                <td style="width:15%">
                                                            ...
                                                            @foreach($products_with_size_data as $row)
                                                                @foreach($row['size'] as $size_row)
                                                                    @if($row['product_name'] == $item_row->name)
                                                                        <option value="{{$size_row['size']}}"
                                                                                data-price="{{$size_row['price']}}" {{$item_row->size == $size_row['size'] ?'selected':''}}>{{$size_row['size']}}</option>
                                                                    @endif
                                                                @endforeach
                                                            @endforeach
                                                        @endif
                                                        <input type="hidden" name="item[{{$key}}][size_id]"
                                                               value="{{$item_row->size_id}}"
                                                               class="item-size-id form-control" required>
...
@section('script')
    ...
    {!! JsValidator::formRequest('App\Http\Requests\SellerInvoiceRequest', '#edit-seller-invoice-form'); !!}
@endsection
